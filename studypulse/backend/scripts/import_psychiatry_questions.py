#!/usr/bin/env python3
"""
Multi-Agent Question Import Script for Psychiatry

This script:
1. Creates Psychiatry subject under NEET PG exam with topics
2. Parses 3 HTML files and extracts questions per topic
3. Imports questions to database with proper topic mapping

HTML Files:
- Prep_RR_Qb_Psychiatry.html
- prepx_Psychiatry.html  
- CEREB_Psychiatry.html
"""
...
HTML_FILES = [
    {
        "path": r"C:\Users\anand\Downloads\Telegram Desktop\Prep_RR_Qb_Psychiatry.html",
        "source": "Prep_RR"
    },
    {
        "path": r"C:\Users\anand\Downloads\Telegram Desktop\prepx_Psychiatry.html",
        "source": "PrepX"
    },
    {
        "path": r"C:\Users\anand\Downloads\Telegram Desktop\CEREB_Psychiatry.html",
        "source": "CEREB"
    }
]
...
def parse_html_file(file_path: str, source_name: str) -> Dict[str, Any]:
    """
    Parse HTML file and extract questions grouped by topic.
    Returns dict with topics and their questions.
    """
    print(f"\n  [PARSING] {source_name}: {file_path}")
    ...
    for idx, srcdoc in enumerate(iframe_matches):
        ...
        if questions:
            ...
            test_id = str(idx + 1)
            topic_name = topics_dict.get(test_id, f"Topic {test_id}")
...
def extract_questions_json(html_content: str) -> List[Dict]:
    ...
    # There may be multiple occurrences - we want the one with actual data (not empty [])
...
def transform_question(q: Dict, topic_id: int) -> Dict:
    ...
    if 'options' in q and isinstance(q['options'], list):
        ...
        for key in ['a', 'b', 'c', 'd', 'e']:
            ...
            if opt_key in q:
                ...
                options_list.append(q[opt_key])
            elif key in q:
                options_dict[key.upper()] = q[key]
                options_list.append(q[key])
    ...
    question_image = question_images[0] if question_images else (q.get('question_image', q.get('image_url', '')) or None)
    explanation_image = explanation_images[0] if explanation_images else (q.get('explanation_image', '') or None)
    ...
    return {
        ...
        "source": q.get('source', 'HTML_IMPORT'),
...
async def get_or_create_subject(session: AsyncSession, exam_id: int) -> Subject:
    """Get or create Psychiatry subject."""
    result = await session.execute(
    ...
    if not subject:
        subject = Subject(
            exam_id=exam_id,
            name=SUBJECT_NAME,
            description="Psychiatry for NEET PG",
            is_active=True
        )
        session.add(subject)
        await session.commit()
...
async def get_or_create_topic(session: AsyncSession, subject_id: int, topic_name: str) -> Topic:
    ...
    if not topic:
        topic = Topic(
            ...
            name=topic_name,
            description=f"Psychiatry topic: {topic_name}",
            is_active=True
        )
        session.add(topic)
        await session.commit()
...
async def import_questions(
    ...
    for q in questions:
        try:
            transformed = transform_question(q, topic.id)
            ...
            question = Question(
                ...
                source=f"{source}_{transformed.get('source', 'HTML')}"
            )
...
async def main():
    ...
    print("\n" + "="*70)
    print("PSYCHIATRY QUESTION IMPORT SCRIPT")
    print("="*70)
    ...
    for file_info in HTML_FILES:
        try:
            parsed = parse_html_file(file_info["path"], file_info["source"])
            all_parsed_data.append(parsed)
        except FileNotFoundError:
            print(f"  [WARNING] File not found: {file_info['path']}")
        except Exception as e:
            print(f"  [ERROR] Failed to parse {file_info['path']}: {e}")
    ...
    print(f"\n  [SUMMARY] Found {len(all_topics)} unique topics across all files")
    ...
    async with async_session() as session:
        ...
        for topic_name in sorted(all_topics):
            topic = await get_or_create_topic(session, subject.id, topic_name)
            ...
            for topic_data in parsed.get("topics", []):
                ...
                if topic_name in topic_map:
                    count = await import_questions(
                        session, topic_map[topic_name], questions, source
                    )
                    total_imported += count
                    print(f"    [{source}] {topic_name}: {count} questions imported")
